...
Instead of directly installing the libraries into the OS space which can casue dependency issues , its safer and preferred to create a virtual environmnet first.
So using built in tool like "venv" is preferred for light weight environments.
In linux and mac OS , you can create a virtual , isolated environemt using the command "python version -m venv ."a name of your wish".
For example since my python version is 3.13 and I would like to name my virtual environment directory just "file'. the command would be ,"python3.13 -m venv .file".
